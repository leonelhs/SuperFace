...
from .generators import my_gen_inference_deep, my_gen_inference_wide
...
def get_artistic_video_colorizer(
    ...
    learn = my_gen_inference_deep(root_folder=root_folder, weights_name=weights_name)
...
def get_stable_video_colorizer(
    ...
    learn = my_gen_inference_wide(root_folder=root_folder, weights_name=weights_name)
    filtr = MasterFilter([ColorizerFilter(learn=learn)], render_factor=render_factor)
    vis = ModelImageVisualizer(filtr, results_dir=results_dir)
    return VideoColorizer(vis)
