...
class ZeroBackground:
    def __init__(self):
        self.deeplab_model = load_model()

    def zero_background(self, path_to_image):
        foreground, bin_mask = remove_background(self.deeplab_model, path_to_image)
        save_image_to = os.path.splitext(path_to_image)[0] + '-zero.png'
        Image.fromarray(foreground).save(save_image_to)
